...
try:  # pragma: no cover - optional GUI dependency
    from PySide6 import QtCore, QtGui, QtOpenGL, QtWidgets  # type: ignore
except Exception:  # pragma: no cover
    QtCore = QtGui = QtOpenGL = QtWidgets = None


if QtOpenGL is not None:  # pragma: no cover - exercised only when GUI deps are installed

    class PreviewWindow(QtOpenGL.QOpenGLWindow):
        ...
        def __init__(self, parent: Any | None = None, *, settings: PreviewSettings | None = None) -> None:
            super().__init__()
            if parent is not None and hasattr(self, "setTransientParent") and hasattr(parent, "windowHandle"):
                try:
                    handle = parent.windowHandle()
                    if handle is not None:
                        self.setTransientParent(handle)
                except Exception:
                    pass
            ...
            self.renderer_mode = "opengl-window-atoms"
        ...
        def _request_interaction_update(self) -> None:
            if self._interaction_repaint_policy == "immediate" and self.isVisible() and hasattr(self, "repaint"):
                self.repaint()
            else:
                self.update()

        def _apply_camera_drag_delta(
            self,
            start: Any,
            end: Any,
            *,
        ...
        def _show_selection_rubber_band(self, start: tuple[float, float], end: tuple[float, float]) -> None:
            ...
            if self._selection_rubber_band is None:
                self._selection_rubber_band = QtWidgets.QRubberBand(QtWidgets.QRubberBand.Shape.Rectangle)
            self._selection_rubber_origin = (float(start[0]), float(start[1]))
            geometry = self._selection_global_rect(start, end)
        ...
        def _selection_global_rect(self, start: tuple[float, float], end: tuple[float, float]) -> Any:
            if QtCore is None:
                return None
            start_point = QtCore.QPoint(int(round(float(start[0]))), int(round(float(start[1]))))
            end_point = QtCore.QPoint(int(round(float(end[0]))), int(round(float(end[1]))))
            try:
                start_point = self.mapToGlobal(start_point)
                end_point = self.mapToGlobal(end_point)
            except Exception:
                pass
            return QtCore.QRect(start_point, end_point).normalized()
    ...
    class PreviewWindow:  # type: ignore[no-redef]
        def __init__(self, *args, **kwargs) -> None:
            raise RuntimeError("PySide6 QtOpenGL is required to instantiate PreviewWindow")


if QtWidgets is not None and QtOpenGL is not None:  # pragma: no cover - exercised only when GUI deps are installed

    class PreviewWindowHost(QtWidgets.QWidget):
        def __init__(self, parent: Any | None = None, *, settings: PreviewSettings | None = None) -> None:
            super().__init__(parent)
            self._window = PreviewWindow(parent=self, settings=settings)
            self._container = QtWidgets.QWidget.createWindowContainer(self._window, self)
            self._window._input_controller.install_on(self, self._container)
            self._axis_overlay_visible = True
            self._axis_overlay = AxisOverlayWidget(self, background=self._window.settings.background)
            layout = QtWidgets.QGridLayout(self)
            layout.setContentsMargins(0, 0, 0, 0)
            layout.setSpacing(0)
            layout.addWidget(self._container, 0, 0)
            ...
            self._window.camera_changed.connect(self._sync_axis_overlay)
        ...
        def set_axis_overlay_visible(self, visible: bool) -> None:
            self._axis_overlay_visible = bool(visible)
            self._window.set_axis_overlay_visible(visible)
            self._axis_overlay.setVisible(bool(visible))
            if visible:
                self._sync_axis_overlay()

        def _sync_axis_overlay(self, *_args: Any) -> None:
            if not self._axis_overlay_visible:
                return
            self._axis_overlay.sync_camera(self._window.current_camera_state())
            self._axis_overlay.raise_()

        def setFocus(self, *args: Any, **kwargs: Any) -> None:
    ...
    class PreviewDetachedHost(QtWidgets.QWidget):
        def __init__(self, parent: Any | None = None, *, settings: PreviewSettings | None = None) -> None:
            super().__init__(parent)
            from atomstudio.app.preview_widget import PreviewWidget

            self._window = PreviewWidget(settings=settings)
            self._window.setWindowTitle("AtomStudio OpenGL Preview")
            self._window.resize(900, 700)
            self._window.show()
            self._window._input_controller.install_on(self)

            label = QtWidgets.QLabel("OpenGL preview is running in a separate window.", self)
    ...
    class PreviewWindowHost:  # type: ignore[no-redef]
        def __init__(self, *args, **kwargs) -> None:
            raise RuntimeError("PySide6 QtOpenGL and QtWidgets are required to instantiate PreviewWindowHost")

    class PreviewDetachedHost:  # type: ignore[no-redef]
        def __init__(self, *args, **kwargs) -> None:
            raise RuntimeError("PySide6 QtWidgets is required to instantiate PreviewDetachedHost")


OpenGLPreviewWindow = PreviewWindow

__all__ = ["OpenGLPreviewWindow", "PreviewDetachedHost", "PreviewWindow", "PreviewWindowHost"]
